...
def get_latent_response_templates(attr_key):
    templates = LATENT_RESPONSES.get(attr_key, GENERIC_LATENT_RESPONSES)
...
class Simulated_User():
    ...
    def __init__(self, train_or_test_or_eval="train"):
        ...
        if train_or_test_or_eval == "train":
            with open("../data_files_that_are_not_big/train_set.json", "r") as f:
                self.pregenerated_responses = json.load(f)
        elif train_or_test_or_eval == "eval":
            with open("../data_files_that_are_not_big/eval_set.json", "r") as f:
                self.pregenerated_responses = json.load(f)
        else:
            with open("../data_files_that_are_not_big/test_set.json", "r") as f:
                self.pregenerated_responses = json.load(f)
